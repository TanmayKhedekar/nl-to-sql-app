...
with tab2:
    ...
    if not tables.empty:
        st.table(tables)
        selected_table = st.selectbox("Select table to preview or manage", tables['name'])
        ...
        if selected_table:
            ...
            # Rename table
            new_name = st.text_input("Rename table", value=selected_table)
